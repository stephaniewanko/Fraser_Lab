...
#while read line; do
  ...
  if [[ -e "multiconformer_model2.pdb" ]]; then
	...
	echo 'None'
      ...
      echo $ligand
      echo $n
      python /wynton/home/fraserlab/swankowicz/190419_Phenix_ensemble/summary_stats_table_creation.py --ligand=$ligand --average_conf_org=${PDB}_baseline_summary_output.txt --average_conf_qfit=${PDB}_multi_summary.txt --pdb_name=$PDB --num_res=$num_residues --pdb_num=$n --refine_log=${PDB}_refine_df.csv --ens_refine_log=${PDB}_${pTLS}_${weights}_ens_refinement_output.csv
    done
  else
   echo 'Qfit not done!'
  fi
