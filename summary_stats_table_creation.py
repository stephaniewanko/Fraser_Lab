...
def create_table(count, pdb, ligand, num_res, org_multi_atoms, org_num_multi, qfit_multi_atoms, qfit_num_multi, refine, ens_refine):
    final_df = pd.read_csv('/wynton/home/fraserlab/swankowicz/190503_Targets/summary_table.csv')
    ...
    combined_df.loc[count, 'Num_Atoms_Multi_Org'] = org_multi_atoms
    combined_df.loc[count, 'Num_Multi_Org'] = org_num_multi
    combined_df.loc[count, 'Num_Atoms_Multi_Qfit'] = qfit_multi_atoms
    combined_df.loc[count, 'Num_Multi_Qfit'] = qfit_num_multi
    combined_df.loc[count, 'Num_Residue'] = num_res
...
def main():
    args = parse_args()
    print('I am here')
    org_multi_atoms, org_num_multi=parse_qfit_summary(args.average_conf_org)
    qfit_multi_atoms, qfit_num_multi=parse_qfit_summary(args.average_conf_qfit)
    print('Creating Table')
    create_table(count=args.pdb_num, pdb=args.pdb_name, ligand=args.ligand, num_res=args.num_res, org_multi_atoms=org_multi_atoms, org_num_multi=org_num_multi, qfit_multi_atoms=qfit_multi_atoms, qfit_num_multi=qfit_num_multi, refine=args.refine_log, ens_refine=args.ens_refine_log)
